...
def test_theme_toggle_is_drawn_with_css_not_text() -> None:
    ...
    assert (
        "aria-label" in webui.frontend.THEME_TOGGLE_JS and "aria-label" in webui.frontend.HEAD_HTML
    )
...
def test_theme_button_has_no_text_label(monkeypatch, tmp_path) -> None:
    ...
    button = next(c for c in demo.blocks.values() if getattr(c, "elem_id", None) == "bb-theme-btn")
    assert button.value == ""
...
def test_bright_dark_controls_share_the_cool_black() -> None:
    ...
    assert (
        webui.theme._bb_vars(webui.theme.BRIGHT)["--bb-chip-bg"] == "#11141C"
    )  # selected chips/rows
