...
def test_sampling_knobs_assets_and_defaults_are_wired() -> None:
    ...
    assert webui.frontend.SAMPLING_KNOBS_FILE.is_file()
    assert "bb-sampling-panel" in webui.theme.BASE_CSS
    assert "bb-view-knobs" in webui.theme.BASE_CSS
...
def test_preset_and_reset_still_drive_the_knob_sliders(demo) -> None:
    ...
    by_id = _by_id(demo)
    reset = next(f for f in demo.fns.values() if f.fn is webui.generate_tab.reset_sampling_values)
...
def test_reset_sampling_values_unchanged() -> None:
    vals = webui.generate_tab.reset_sampling_values()
    assert len(vals) == 15  # 14 params + preset label
    assert vals[0] == webui.runtime.ABC_DEFAULTS["temperature"]
    assert vals[13] == webui.runtime.SEM_DEFAULTS["max_tokens"]
