...
def test_protocol_objects_and_fields():
    ...
    assert (
        request.guidance == 1.5 and SongRequest(style="a", lyrics="b", cot="off").guidance == 1.01
    )
...
def test_saved_artifacts_are_what_the_library_reads(tmp_path):
    ...
    request = SongRequest(
        style="English, test tone", lyrics="[Verse]\nHello", cot="full", seed=7, id="demo"
    )
    abc = (
        "X:1\nT:\nM:4/4\nL:1/32\nQ:1/4=120\n"
    ...
    plan = SymbolicPlan(
        request, abc, [1, 2, 3], [9, 8, 7], {"seconds": 1.0, "output_tokens": 3}, False
    )
    semantic = SemanticResult(
        plan, [4, 5, 6], {"seconds": 2.0, "output_tokens": 3, "output_tps": 1.5}, False
    )
    ...
    assert (
        details["result"]["status"] == "complete" and details["config"]["model_dtype"] == "bfloat16"
    )
    ...
    assert json.loads((directory / "result.json").read_text())["audio_seconds"] == pytest.approx(
        1.0
    )


def test_cuda_graph_still_selects_flash_attention_by_op_presence_only():
    """Guards the UI's torch-eager fallback (``webui.runtime.effective_backend``).
    ...
    retire = (
        "Upstream now checks FlashAttention availability itself. Retire the workaround: "
        "remove webui.runtime.effective_backend / FLASH_FALLBACK_NOTE and the call in "
        "webui.runtime.load_pipeline, adapter.cuda_flash_attention_usable, "
        "tests/test_flash_fallback.py, the README paragraph 'CUDA without FlashAttention', "
        "and then this test."
    )
    assert not any(
        name in source for name in ("can_use_flash_attention", "is_flash_attention_available")
    ), retire
    assert 'hasattr(torch.ops.aten, "_flash_attention_forward")' in source, (
        ...
        "torch-eager fallback in webui.runtime.effective_backend is still needed."
    )
