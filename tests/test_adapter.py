...
class LegacyPipeline:
    ...
    def __call__(self, *, style, lyrics, cot, seed, abc, cfg_scale, id, abc_sampling,
                 semantic_sampling, cancelled, on_token):
        import yue2.nar as nar
        from yue2.modeling_vae import YuE2VAE
        self.calls.append((style, lyrics, cot, seed, id))
        latents = nar.synthesize(None, [1], [2], seed, steps=4, context=1, offload_ar=False,
                                 cancelled=cancelled, on_progress=None)
        audio = YuE2VAE().decode_tiled(latents, core_frames=1, halo_frames=16, output_device="cpu",
                                       on_progress=None)
    ...
    def decode(self, latents, *, full=False, vae=None):
        ...
        if full:
            return "audio-full"
        return YuE2VAE().decode_tiled(latents, core_frames=1, halo_frames=16, output_device="cpu",
                                      on_progress=None)
...
def test_legacy_pipeline_gets_progress_through_wrapped_internals(monkeypatch):
    ...
    result = adapter.generate(pipe, Request, abc_sampling="a", semantic_sampling="s",
...
def test_legacy_pipeline_without_callback_is_not_wrapped(monkeypatch):
    nar, vae_class = install_fake_yue2(monkeypatch)
    original = (nar.synthesize, vae_class.decode_tiled)
    assert adapter.generate(LegacyPipeline(), Request, abc_sampling="a", semantic_sampling="s") == ("latents", "audio")
...
def test_native_pipeline_receives_the_keyword(monkeypatch):
    ...
    assert adapter.generate(pipe, Request, abc_sampling="a", semantic_sampling="s",
                            on_progress=lambda *a: seen.append(a)) == "native"
...
def test_decode_paths(monkeypatch):
    install_fake_yue2(monkeypatch)
    seen = []
    assert adapter.decode(LegacyPipeline(), "z", on_progress=lambda d, t: seen.append((d, t))) == "audio"
    assert seen == [(1, 2), (2, 2)]
    seen.clear()
    assert adapter.decode(LegacyPipeline(), "z", full=True, on_progress=lambda d, t: seen.append((d, t))) == "audio-full"
    assert seen == [(1, 1)]
    seen.clear()
    assert adapter.decode(NativePipeline(), "z", on_progress=lambda d, t: seen.append((d, t))) == "native-audio"
    assert seen == [(2, 2)]
...
def test_wrapping_is_restored_after_an_exception(monkeypatch):
    ...
    with pytest.raises(InterruptedError):
        adapter.generate(Failing(), Request, abc_sampling="a", semantic_sampling="s",
...
def test_doctor_command_shape():
    cmd = adapter.doctor_command("m-a-p/YuE2-3B", "m-a-p/YuE2-Vae", revision="r1", offline=True, verify=True)
    assert cmd[1:] == ["-m", "yue2.cli", "doctor", "--model", "m-a-p/YuE2-3B", "--vae", "m-a-p/YuE2-Vae",
                       "--revision", "r1", "--offline", "--verify-hashes"]
