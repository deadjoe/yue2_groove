...
def test_cover_strip_and_send_flow() -> None:
    ...
    (score_update, cot_update, style_update, lyrics_update, score_input, tab_update,
     status) = webui.cover_tab.cover_send_to_generate(BASE_ABC, "melody-full",
                                            "English jazz", "[Verse]\nla", "both")
    ...
    (score_update, cot_update, style_update, lyrics_update, _score_input, _tab,
     status) = webui.cover_tab.cover_send_to_generate(BASE_ABC, "full", "", "  ", "both")
...
def test_cover_transcribe_yields_result_and_frees_the_lock(monkeypatch, tmp_path: Path) -> None:
    ...
    yields = list(webui.cover_tab.cover_transcribe(str(audio), "melody-full", 0, "m-a-p/SheetSage2",
    ...
    assert list(webui.cover_tab.cover_transcribe(str(audio), "melody-full", 0, "m", "auto", "auto", "",
                                       "", False, False))
...
def test_cover_transcribe_honours_the_transcriptions_override(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(webui.cover_tab.sheetsage_adapter, "transcribe", fake_transcribe)
    list(webui.cover_tab.cover_transcribe(str(audio), "melody-full", 0, "m", "auto", "auto", "", "", False,
                                    False))
...
def test_cover_transcribe_refuses_without_audio_and_when_busy() -> None:
    with pytest.raises(gr.Error, match="Upload a source audio"):
        next(webui.cover_tab.cover_transcribe("", "melody-full", 0, "", "auto", "auto", "", "", False, False))
    webui.runtime.RUNNING.acquire()
    try:
        yields = list(webui.cover_tab.cover_transcribe("/tmp/x.wav", "melody-full", 0, "", "auto",
                                             "auto", "", "", False, False))
...
def test_edit_freeze_and_check_guards(isolated_runs: Path) -> None:
    ...
    record, info, status = webui.edit_tab.edit_freeze("20260901-120000-source", "20260901-120000-source")
    ...
    output, state = webui.edit_tab.edit_check(BASE_ABC, BASE_ABC, "both", False, record, "exact", False)
    assert json.loads(output)["match"] is True and state["sha256"]
    output, state = webui.edit_tab.edit_check(BASE_ABC, CHORD_FREE, "both", False, record, "exact", False)
...
def test_edit_generate_guards(isolated_runs: Path, monkeypatch) -> None:
    ...
    with pytest.raises(gr.Error, match="did not pass"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(
            check_state={"sha256": edit_flow.sha256_text(edit_flow.clean_abc(BASE_ABC)),
    ...
    with pytest.raises(gr.Error, match="Load a source work"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(baseline_abc="", baseline_state=None)))
    # the override only lets a *failing* check through; freeze and the check stay mandatory
    with pytest.raises(gr.Error, match="Load a source work"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(baseline_abc="", baseline_state=None,
                                                     allow_changes=True)))
    with pytest.raises(gr.Error, match="Freeze the baseline"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(baseline_state=None, allow_changes=True)))
    with pytest.raises(gr.Error, match="Run CHECK INVARIANTS"):
    ...
    failing = {"sha256": edit_flow.sha256_text(edit_flow.clean_abc(BASE_ABC)),
               ...
               "voices": "both", "allow_tempo_change": False}
    yields = list(webui.edit_tab.edit_generate(*edit_generate_args(check_state=failing, allow_changes=True)))
...
def test_sampling_summary_mirrors_the_shared_sliders() -> None:
    text = webui.cover_tab.sampling_summary(.7, .9, 30, 1.005, 100, 32, 4096,
...
def test_cover_transcribe_cancel_explains_the_stopped_worker(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(webui.cover_tab.sheetsage_adapter, "transcribe", fake_transcribe)
    yields = list(webui.cover_tab.cover_transcribe(str(audio), "melody-full", 0, "m", "auto", "auto", "",
                                         "", True, False))
    status = yields[-1][2]
...
def test_sampling_mirror_load_event_returns_one_value_per_output() -> None:
    ...
    events = [f for f in demo.fns.values()
              if getattr(f.fn, "__name__", "") == "sampling_summary_pair"]
    assert len(events) == 1
...
def test_baselines_are_not_library_works(isolated_runs: Path) -> None:
    ...
    assert all(not rel.startswith("baselines/") for rel in
               (rel for _label, rel in webui.edit_tab.edit_choices()["choices"]))


def test_cover_load_fills_from_a_work_and_from_a_transcription(isolated_runs: Path) -> None:
    make_work(isolated_runs)
    abc_update, style_update, lyrics_update, status = webui.cover_tab.cover_load("20260901-120000-source")
...
def test_cover_send_to_edit_hands_over_the_source(isolated_runs: Path) -> None:
    ...
    (abc_update, baseline, style_update, lyrics_update, rel, check_state, baseline_state,
     info, status, source_update, source_path, tab_update) = webui.cover_tab.cover_send_to_edit(
...
def test_library_flow_back_and_open_last_in_library(isolated_runs: Path) -> None:
    ...
    for call in (webui.library_tab.library_open_in_edit, webui.library_tab.library_use_in_cover,
...
def test_cover_detect_python_uses_a_documented_location(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.delenv("YUE2_GROOVE_SHEETSAGE_PYTHON", raising=False)
    monkeypatch.setattr(webui.cover_tab, "_sheetsage_python_candidates", lambda: [tmp_path / "nope", fake])
    ...
    monkeypatch.setattr(webui.cover_tab, "_sheetsage_python_candidates", lambda: [tmp_path / "nope"])
