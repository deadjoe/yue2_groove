...
def test_score_panel_marks_the_textarea_label() -> None:
    html = webui.frontend.score_panel("COVER ABC", "nothing here")
...
def test_cover_strip_and_send_flow() -> None:
    stripped, status = webui.cover_tab.cover_strip(BASE_ABC, "both")
    assert '"C"' not in stripped and "verified" in status

    (score_update, cot_update, style_update, lyrics_update, score_input, tab_update, status) = (
        webui.cover_tab.cover_send_to_generate(
            BASE_ABC, "melody-full", "English jazz", "[Verse]\nla", "both"
        )
    )
    ...
    (score_update, cot_update, style_update, lyrics_update, _score_input, _tab, status) = (
        webui.cover_tab.cover_send_to_generate(BASE_ABC, "full", "", "  ", "both")
    )
    ...
    with pytest.raises(gr.Error, match="Cannot send"):
        webui.cover_tab.cover_send_to_generate("   ", "melody-full", "", "", "both")
...
def test_cover_check_environment_reports_missing_configuration(monkeypatch) -> None:
    ...
    message = webui.cover_tab.cover_check_environment()
...
def test_cover_transcribe_yields_result_and_frees_the_lock(monkeypatch, tmp_path: Path) -> None:
    ...
    transcript = webui.runtime.RUNS / "transcriptions" / "reference"
    ...
    def fake_transcribe(audio_path, **kwargs):
        ...
        return {
            ...
            "task": "melody-full",
            "melody_only": True,
            "output_dir": str(transcript),
            "device": "cpu",
            "dtype": "fp32",
        }

    monkeypatch.setattr(webui.cover_tab.sheetsage_adapter, "transcribe", fake_transcribe)
    yields = list(
        webui.cover_tab.cover_transcribe(
            str(audio),
            "melody-full",
            0,
            "m-a-p/SheetSage2",
    ...
    assert seen["output_dir"].parent == webui.runtime.RUNS / "transcriptions"
    # the lock was released: a second transcription can start
    assert list(
        webui.cover_tab.cover_transcribe(
            str(audio), "melody-full", 0, "m", "auto", "auto", "", "", False, False
        )
    )
...
def test_cover_transcribe_refuses_without_audio_and_when_busy() -> None:
    with pytest.raises(gr.Error, match="Upload a source audio"):
        next(
            webui.cover_tab.cover_transcribe(
                "", "melody-full", 0, "", "auto", "auto", "", "", False, False
            )
        )
    ...
    try:
        yields = list(
            webui.cover_tab.cover_transcribe(
                "/tmp/x.wav", "melody-full", 0, "", "auto", "auto", "", "", False, False
            )
        )
...
def test_edit_load_fills_the_baseline(isolated_runs: Path) -> None:
    ...
    (style, lyrics, abc, baseline_abc, rel, check_state, baseline_state, info, status) = (
        webui.edit_tab.edit_load("20260901-120000-source")
    )
    ...
    with pytest.raises(gr.Error, match="Pick a source work"):
        webui.edit_tab.edit_load("")


def test_edit_freeze_and_check_guards(isolated_runs: Path) -> None:
    make_work(isolated_runs)
    with pytest.raises(gr.Error, match="press LOAD first"):
        webui.edit_tab.edit_freeze("20260901-120000-source", "20260901-129999-other")
    record, info, status = webui.edit_tab.edit_freeze(
        "20260901-120000-source", "20260901-120000-source"
    )
    ...
    with pytest.raises(gr.Error, match="Load a source work"):
        webui.edit_tab.edit_freeze("")

    with pytest.raises(gr.Error, match="Freeze the baseline"):
        webui.edit_tab.edit_check(BASE_ABC, BASE_ABC, "both", False, None, "exact", False)
    output, state = webui.edit_tab.edit_check(
        BASE_ABC, BASE_ABC, "both", False, record, "exact", False
    )
    assert json.loads(output)["match"] is True and state["sha256"]
    output, state = webui.edit_tab.edit_check(
        BASE_ABC, CHORD_FREE, "both", False, record, "exact", False
    )
    assert json.loads(output)["match"] is True  # chord-only edits pass
    with pytest.raises(gr.Error, match="Load a source work"):
        webui.edit_tab.edit_check("", BASE_ABC, "both", False, record, "exact", False)
...
class FakeSong:
    ...
    def __init__(self):
        self.timing = {
            "nar_seconds": 1.0,
            "vae_seconds": 0.5,
            "semantic": {"output_tps": 9.0},
            "abc": {"output_tokens": 10},
        }
...
def edit_generate_args(**overrides):
    args = dict(  # noqa: C408 — test fixture builder
        ...
        cfg_scale=0,
        abc_text=BASE_ABC,
        baseline_abc=BASE_ABC,
        source_rel="20260901-120000-source",
        check_state={
            "sha256": edit_flow.sha256_text(edit_flow.clean_abc(BASE_ABC)),
            "match": True,
            "result": {"match": True},
            "voices": "both",
        ...
        allow_changes=False,
        baseline_state={"schema": "yue2-groove-baseline-v1", "baseline": {"path": "/tmp/baseline"}},
        abc_temp=0.7,
        abc_p=0.9,
        abc_k=30,
        abc_rep=1.005,
...
def test_edit_generate_guards(isolated_runs: Path, monkeypatch) -> None:
    with pytest.raises(gr.Error, match="cannot be used"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(abc_text="   ")))
    with pytest.raises(gr.Error, match="Run CHECK INVARIANTS"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(check_state={})))
    with pytest.raises(gr.Error, match="did not pass"):
        next(
            webui.edit_tab.edit_generate(
                *edit_generate_args(
                    check_state={
                        "sha256": edit_flow.sha256_text(edit_flow.clean_abc(BASE_ABC)),
                        "match": False,
                        "result": {"differences": ["Vocal: sounding notes differ"]},
                        "voices": "both",
                        "allow_tempo_change": False,
                    }
                )
    ...
    with pytest.raises(gr.Error, match="Freeze the baseline"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(baseline_state=None)))
    with pytest.raises(gr.Error, match="Load a source work"):
        next(
            webui.edit_tab.edit_generate(*edit_generate_args(baseline_abc="", baseline_state=None))
        )
    # the override only lets a *failing* check through; freeze and the check stay mandatory
    with pytest.raises(gr.Error, match="Load a source work"):
        next(
            webui.edit_tab.edit_generate(
                *edit_generate_args(baseline_abc="", baseline_state=None, allow_changes=True)
            )
        )
    with pytest.raises(gr.Error, match="Freeze the baseline"):
        next(
            webui.edit_tab.edit_generate(
                *edit_generate_args(baseline_state=None, allow_changes=True)
            )
        )
    with pytest.raises(gr.Error, match="Run CHECK INVARIANTS"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(check_state={}, allow_changes=True)))
    ...
    yields = list(
        webui.edit_tab.edit_generate(*edit_generate_args(check_state=failing, allow_changes=True))
    )
...
def test_edit_generate_happy_path_writes_a_manifest(isolated_runs: Path, monkeypatch) -> None:
    monkeypatch.setattr(webui.runtime, "get_pipe", lambda *a, **k: (object(), "note"))
    monkeypatch.setattr(webui.runtime, "run_generation", fake_run_generation)
    yields = list(webui.edit_tab.edit_generate(*edit_generate_args()))
...
def test_edit_generate_event_targets_the_result_box_not_the_editor(isolated_runs: Path) -> None:
    ...
    events = [f for f in demo.fns.values() if getattr(f.fn, "__name__", "") == "edit_generate"]
    assert len(events) == 1
...
def test_edit_compare_requires_both_sides(isolated_runs: Path) -> None:
    with pytest.raises(gr.Error, match="Load a source work"):
        webui.edit_tab.edit_compare("", "/tmp/x")
    with pytest.raises(gr.Error, match="Generate an edited version"):
        webui.edit_tab.edit_compare("20260901-120000-source", "")
...
def test_edit_source_choices_skip_non_works(isolated_runs: Path) -> None:
    ...
    scanned = {Path(p).name for p in webui.runtime.scan_runs()}
    ...
    choices = webui.edit_tab.edit_choices()["choices"]
...
def test_score_panel_escapes_data_attributes() -> None:
    html_text = webui.frontend.score_panel('OP"EN', 'quote " and <tag> & more', elem_id="bb-x")
...
def test_sampling_summary_mirrors_the_shared_sliders() -> None:
    text = webui.cover_tab.sampling_summary(
        0.7, 0.9, 30, 1.005, 100, 32, 4096, 1.0, 0.95, 100, 1.2, 50, 200, 9000
    )
...
def test_cover_generate_runs_directly_from_the_cover_tab(isolated_runs: Path, monkeypatch) -> None:
    captured = {}
    ...
    yields = list(webui.cover_tab.cover_generate(*cover_generate_args()))
    ...
    list(webui.cover_tab.cover_generate(*cover_generate_args(task="full")))
    assert captured["cot"] == "full" and '"C"' in captured["abc"]

    with pytest.raises(gr.Error, match="score-conditioned"):
        next(webui.cover_tab.cover_generate(*cover_generate_args(abc_text="   ")))


# ── comparison builders ──────────────────────────────────────────────────


def test_edit_compare_feeds_the_source_and_last_run_to_the_builder(
    isolated_runs: Path, monkeypatch
) -> None:
    make_work(isolated_runs)
    captured = {}
    ...
    monkeypatch.setattr(webui.tools_tab, "make_comparison", fake_make_comparison)
    result = webui.edit_tab.edit_compare("20260901-120000-source", "/tmp/runs/edit-1")
...
def test_make_comparison_builds_a_real_listening_bundle(isolated_runs: Path) -> None:
    ...
    for name in ("20260901-120000-a", "20260901-120001-b"):
        ...
        (d / "request.json").write_text(
            json.dumps({"id": name, "style": "test", "lyrics": "la", "cot": "full", "seed": 1}),
            encoding="utf-8",
        )
        (d / "audio.flac").write_bytes(b"fLaC" + b"\x00" * 64)
        sources.append(str(d))

    html_path, link, status = webui.tools_tab.make_comparison("\n".join(sources))
...
def test_cover_transcribe_cancel_explains_the_stopped_worker(monkeypatch, tmp_path: Path) -> None:
    ...
    yields = list(
        webui.cover_tab.cover_transcribe(
            str(audio), "melody-full", 0, "m", "auto", "auto", "", "", True, False
        )
    )
    status = yields[-1][2]
...
def test_sampling_mirror_load_event_returns_one_value_per_output() -> None:
    ...
    pair = webui.cover_tab.sampling_summary_pair(*range(14))
    ...
    events = [
        f for f in demo.fns.values() if getattr(f.fn, "__name__", "") == "sampling_summary_pair"
    ]
    assert len(events) == 1
...
def test_baselines_are_not_library_works(isolated_runs: Path) -> None:
    ...
    assert all(
        not rel.startswith("baselines/")
        for rel in (rel for _label, rel in webui.edit_tab.edit_choices()["choices"])
    )


def test_cover_load_fills_from_a_work_and_from_a_transcription(isolated_runs: Path) -> None:
    make_work(isolated_runs)
    abc_update, style_update, lyrics_update, status = webui.cover_tab.cover_load(
        "20260901-120000-source"
    )
    assert abc_update["value"] == BASE_ABC.strip()
    assert style_update["value"].startswith("English") and lyrics_update["value"].startswith(
        "[Verse]"
    )
    ...
    (d / "result.json").write_text(
        json.dumps({"task": "melody-full", "abc": "X:1"}), encoding="utf-8"
    )
    ...
    with pytest.raises(gr.Error, match="no ABC"):
        ...
        webui.cover_tab.cover_load("20260901-140000-empty")
...
def test_cover_send_to_edit_hands_over_the_source(isolated_runs: Path) -> None:
    ...
    (
        ...
        source_path,
        tab_update,
    ) = webui.cover_tab.cover_send_to_edit(edited, "20260901-120000-source", "jazz", "")
    ...
    with pytest.raises(gr.Error, match="Transcribe or load"):
        webui.cover_tab.cover_send_to_edit("  ", "20260901-120000-source", "", "")
    with pytest.raises(gr.Error, match="LOAD a source work"):
        webui.cover_tab.cover_send_to_edit(BASE_ABC, "", "", "")
    with pytest.raises(gr.Error, match="no longer exists"):
        webui.cover_tab.cover_send_to_edit(BASE_ABC, "does-not-exist", "", "")
...
def test_cover_choices_lists_works_and_transcriptions(isolated_runs: Path) -> None:
    ...
    values = {value for _label, value in webui.cover_tab.cover_choices()["choices"]}
...
def test_request_texts_requires_explicit_input() -> None:
    with pytest.raises(gr.Error, match="STYLE and LYRICS empty"):
        webui.runtime.request_texts("", "  ")
    with pytest.raises(gr.Error, match="LYRICS empty"):
        webui.runtime.request_texts("pop", "")
    assert webui.runtime.request_texts("  pop ", " la ") == ("pop", "la")
    style, lyrics = webui.runtime.request_texts("", "", fallback=True)
    assert (style, lyrics) == (webui.runtime.EXAMPLE_STYLE, webui.runtime.EXAMPLE_LYRICS)


def test_empty_style_is_refused_on_generate_paths() -> None:
    with pytest.raises(gr.Error, match="STYLE empty"):
        next(webui.cover_tab.cover_generate(*cover_generate_args(style="   ")))
    with pytest.raises(gr.Error, match="STYLE empty"):
        next(webui.edit_tab.edit_generate(*edit_generate_args(style="")))
...
def test_cover_generate_passes_melody_voices_through(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(webui.runtime, "get_pipe", lambda *a, **k: (object(), "note"))
    monkeypatch.setattr(webui.runtime, "run_generation", fake_run_generation)
    list(webui.cover_tab.cover_generate(*cover_generate_args(keep_voice="Vocal")))
...
def test_library_flow_back_and_open_last_in_library(isolated_runs: Path) -> None:
    ...
    outs = webui.library_tab.library_open_in_edit("20260901-120000-source")
    assert len(outs) == 12
    ...
    outs = webui.library_tab.library_use_in_cover("transcriptions/20260901-130000-reference")
    ...
    outs = webui.library_tab.open_last_in_library(str(run))  # absolute path accepted
    assert len(outs) == 11 and outs[0]["value"] == ["20260901-140000-fresh"]
    assert "bb-player" in outs[1] and outs[8]["selected"] == "library"

    for call in (
        webui.library_tab.library_open_in_edit,
        webui.library_tab.library_use_in_cover,
        webui.library_tab.open_last_in_library,
    ):
        with pytest.raises(gr.Error, match="Pick or generate"):
            call("")
    with pytest.raises(gr.Error, match="outside the runs directory"):
        webui.library_tab.open_last_in_library("/tmp/somewhere-else")
...
def test_generate_hands_the_run_to_the_flow_back_buttons(monkeypatch, isolated_runs) -> None:
    ...
    monkeypatch.setattr(webui.runtime, "get_pipe", lambda *a, **k: (object(), "note"))
    monkeypatch.setattr(webui.runtime, "run_generation", fake_run_generation)
    yields = list(webui.generate_tab.generate(*generate_args()))
...
def test_cover_detect_python_uses_a_documented_location(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.delenv("YUE2_GROOVE_SHEETSAGE_PYTHON", raising=False)
    monkeypatch.setattr(
        webui.cover_tab, "_sheetsage_python_candidates", lambda: [tmp_path / "nope", fake]
    )
    message = webui.cover_tab.cover_detect_python()
    assert str(fake) in message and os.environ["YUE2_GROOVE_SHEETSAGE_PYTHON"] == str(fake)
    monkeypatch.setattr(
        webui.cover_tab, "_sheetsage_python_candidates", lambda: [tmp_path / "nope"]
    )
    assert "No SheetSage2 venv found" in webui.cover_tab.cover_detect_python()
...
def test_publish_current_returns_band_then_bridge(isolated_runs: Path) -> None:
    ...
    band, bridge = webui.song_view.publish_current(path)
    ...
    band, bridge = webui.song_view.publish_current(str(isolated_runs / "missing"))
...
def test_handoffs_set_the_current_work(isolated_runs: Path) -> None:
    ...
    edit_outs = webui.library_tab.library_open_in_edit("20260901-120000-source")
    assert edit_outs[-2] == source and edit_outs[-1]["selected"] == "edit"

    cover_outs = webui.library_tab.library_use_in_cover("20260901-120000-source")
    assert cover_outs[-2] == source and cover_outs[-1]["selected"] == "cover"

    library_outs = webui.library_tab.open_last_in_library("20260901-120000-source")
    ...
    send_outs = webui.cover_tab.cover_send_to_edit(abc, "20260901-120000-source", "", "")
    assert send_outs[-2] == source and send_outs[-1]["selected"] == "edit"

    # and the bridge value really lights the band
    band, bridge = webui.song_view.publish_current(send_outs[-2])
